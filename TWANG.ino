...
void tickEnemies() {
    for (uint8_t i = 0; i<ENEMY_COUNT; i++) {
        if (enemyPool[i].Alive()) {
            ...
            // Hit player?
            if ((enemyPool[i].playerSide == 1 && enemyPool[i]._pos <= playerPosition) ||
...
void tickBoss() {
    ...
    if (boss.Alive()) {
        ...
        // CHECK FOR ATTACK
        if (attacking) {
            if ((getLED(playerPosition + (attack_width / 2)) >= getLED(boss._pos - BOSS_WIDTH / 2) &&
...
void tickSpawners() {
    ...
    for(uint8_t s = 0; s < SPAWN_COUNT; s++) {
        if(spawnPool[s].Alive() && spawnPool[s]._activate < mm) {
            if(spawnPool[s]._lastSpawned + spawnPool[s]._rate < mm ||
               spawnPool[s]._lastSpawned == 0) {
...
void tickConveyors() {
    ...
    for(i = 0; i < CONVEYOR_COUNT; i++) {
        if (conveyorPool[i]._alive) {
            ...
            if (playerPosition > conveyorPool[i]._startPoint &&
               playerPosition < conveyorPool[i]._endPoint) {
...
void drawAttack() {
    ...
    for (int i = getLED(playerPosition - (attack_width / 2)) + 1;
             i <= getLED(playerPosition + (attack_width / 2)) - 1; i++) {
        leds[i] = CRGB(0, 0, n);
    }
    if (n > 90) {
        n = 255;
...
// ----------- JOYSTICK ------------
// ---------------------------------
ThreeButtonJoystick myJoystick = ThreeButtonJoystick(2, 3, 4);
void getInput() {
    ...
    myJoystick.readInputs();
    joystickTilt = myJoystick.angle;
    joystickWobble = myJoystick.buttonPressed ? ATTACK_THRESHOLD + 1 : 0;
}
